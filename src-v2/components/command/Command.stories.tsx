...
const LOAD_MORE_ITEMS = Array.from({ length: 36 }).map(
  (_, index) => ({
    id: `loaded-${index + 1}`,
    label: `項目 ${index + 1}`,
  }),
...
export const Basic: Story = {
  render: function Render() {
    ...
    return (
      <Command label="Inline command" className="h-[300px] w-[194px]">
        <Command.Input
          placeholder="輸入關鍵字"
          value={search}
          onValueChange={setSearch}
...
export const WithTrigger: Story = {
  render: function Render() {
    ...
    return (
      <div className="w-[194px] pb-[300px]">
        <button
          type="button"
          aria-expanded={open}
...
export const LoadMore: Story = {
  render: function Render() {
    ...
    const loadMore = () => {
      ...
      window.setTimeout(() => {
        setCount((prev) =>
          Math.min(prev + PAGE_SIZE, LOAD_MORE_ITEMS.length),
        );
        setIsLoading(false);
      }, 800);
    };
    ...
    return (
      <Command
        label="Load more command"
        className="h-[300px] w-[194px]"
      >
        <Command.Input
          placeholder="輸入關鍵字"
          value={search}
