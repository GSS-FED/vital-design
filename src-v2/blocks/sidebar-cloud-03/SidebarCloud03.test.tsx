...
describe('CloudSidebar 03', () => {
  beforeEach(() => {
    mockedUseIsMobile.mockReturnValue(false);
  });

  it('renders brand, Add, and nav without Overview label or footer', () => {
    renderCloud();
  ...
  it('opens an add menu from the header Add button', async () => {
    ...
    expect(
      screen.getByRole('menuitem', { name: 'New client' }),
    ).toBeInTheDocument();
    expect(
  ...
  it('keeps an icon-only Add button when the sidebar is icon-collapsed', () => {
    renderCloud({
      defaultOpen: false,
  ...
  it('opens nested items in a dropdown instead of an inline submenu', async () => {
    ...
    expect(
      screen.getByRole('menuitem', { name: 'Archived' }),
    ).toBeInTheDocument();
