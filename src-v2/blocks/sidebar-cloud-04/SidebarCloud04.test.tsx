...
describe('CloudSidebar 04', () => {
  ...
  it('renders brand, secondary nav, and user footer without Add', () => {
    ...
    expect(support).toContainHTML(
      'group-data-[collapsible=icon]:hidden',
    );
    expect(support).toContainHTML('viewBox="0 0 24 24"');
    ...
    const groups = queryAllByAttribute(
      'data-slot',
      document.body,
      'sidebar-group',
    );
    expect(groups[groups.length - 1]?.className).toMatch(
      /\bmt-auto\b/,
    );
    expect(groups[groups.length - 1]?.className).toMatch(/\bpb-8\b/);
  });
