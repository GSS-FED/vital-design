...
export function ColorSwatch({ name, value, utility }: ColorEntry) {
  return (
    <div className="flex items-center gap-3 rounded-md border border-grayscale-opacity-200 bg-white p-3">
      <div
        className="size-10 shrink-0 rounded-md border border-grayscale-opacity-300/60"
        style={{ background: `var(${name})` }}
      />
      <div className="min-w-0 flex-1">
        <div className="font-mono text-xs text-grayscale-opacity-900">
          {name}
        </div>
        {value ? (
          <div className="font-mono text-xs text-grayscale-opacity-600">
            {value}
          </div>
        ) : null}
        {utility ? (
          <div className="font-mono text-[11px] text-grayscale-opacity-500">
            {utility}
          </div>
        ) : null}
      </div>
  ...
  return (
    <div className="not-prose my-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {tokens.map((token) => (
        <div
          key={token.name}
          className="flex items-center gap-3 rounded-md border border-grayscale-opacity-200 bg-white p-3"
        >
          <div
          ...
          <div className="min-w-0 flex-1">
            <div className="text-sm font-medium text-grayscale-opacity-800">
              {token.name}
            </div>
            <div className="font-mono text-[11px] text-grayscale-opacity-500">
              --avatar-{token.name}-*
  ...
  return (
    <div className="not-prose my-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
      {tokens.map((token) => (
        <div
          key={token.name}
          className="flex flex-col items-center gap-2 rounded-md border border-grayscale-opacity-200 bg-white p-4"
        >
          <div
          ...
          <div className="text-center">
            ...
            <div className="font-mono text-[11px] text-grayscale-opacity-600">
              {token.value}
            </div>
            {token.utility ? (
              <div className="font-mono text-[11px] text-grayscale-opacity-500">
  ...
  return (
    <div className="not-prose my-4 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {tokens.map((token) => (
        <div
          key={token.name}
          className="flex flex-col items-center gap-3 rounded-md bg-grayscale-opacity-100 p-6"
        >
          <div
  ...
  return (
    <div className="not-prose my-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
      {tokens.map((token) => (
        <div
          key={token.name}
          className="flex flex-col gap-2 rounded-md border border-grayscale-opacity-200 bg-white p-3"
        >
          <div
...
export function TokenSection({
  ...
  return (
    <div className="my-6">
      {title ? (
        <h3 className="text-base font-semibold text-grayscale-opacity-900">
          {title}
        </h3>
      ) : null}
      {description ? (
        <p className="mt-1 text-sm text-grayscale-opacity-700">
          {description}
        </p>
      ) : null}
      {children}
